...
QuestModel =  module.exports = function(environment) {
    ...
    self.fillDatatable = function(start, count, userId, userIP, sToken, callback) {
        ...
        topicDriver.listInstanceTopics(Constants.QUEST_TYPE, start, count, userId, userIP, sToken, function bmF(err, rslt) {
    ...
    self.create = function(json, userId, userIP, sToken, callback) {
        ...
        var pivots = CommonModel.jsonBallToPivots(json),
            lang = json.language;
        if (!lang) { lang = "en";}
        CommonModel.createTopicInstance(null, Constants.QUEST_TYPE, userId, json.title, json.body, lang,
            Constants.QUEST, Constants.QUEST_SM, false, null, pivots,
